...
imp='use Aws\\TranscribeService\\TranscribeServiceClient;'
anchor='use Aws\\Translate\\TranslateClient;'
if imp not in text: text=text.replace(anchor,anchor+'\n'+imp,1)
method=r'''
...
'''
if 'public static function getTranscribe()' not in text:
    pos=text.rfind('\n}'); text=text[:pos]+method+text[pos:]
config.write_text(text,encoding='utf-8')

write(SRC/'Aws/TranscriptionFileService.php', r'''<?php
...
write(SRC/'Http/Controller/TranscriptionController.php', r'''<?php
declare(strict_types=1);
...
for filename,method in {'transcribir_iniciar.php':'start','transcribir_estado.php':'status'}.items():
    write(DRIVE/filename,f'''<?php\ndeclare(strict_types=1);\nrequire_once __DIR__ . '/app_bootstrap.php';\n(new \\ArcadeCloud\\Drive\\Http\\Controller\\TranscriptionController(\n    \\ArcadeCloud\\Drive\\Core\\ApplicationKernel::app(),\n    \\ArcadeCloud\\Drive\\Http\\Request::fromGlobals()\n))->{method}();\n''')
print('Transcribe migrated')
