...
Dos archivos con el mismo nombre físico son válidos si están en carpetas distintas del mismo usuario:
...
`install_node_sync_service.sh` ejecuta esta migración con el mismo usuario y `EnvironmentFile` del worker antes de instalar/usar la sincronización completa del nodo.
