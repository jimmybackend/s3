 ...
 *
 * DriveActivityEvents conserva el CorrelationId hash(jobName). Amazon conserva
 * los jobName durante su periodo de retención. Por eso podemos listar los jobs
 * en AWS, recalcular el mismo CorrelationId y localizar el evento pendiente sin
 * guardar nombres de trabajo o secretos adicionales en una tabla nueva.
 */
final class TranscriptionReconciler
{
        ...
        foreach (self::AWS_STATUSES as $awsStatus) {
            ...
            do {
                ...
                foreach ((array)($page['TranscriptionJobSummaries'] ?? []) as $summary) {
                    ...
                    if ($correlation === null || !isset($byCorrelation[$correlation])) {
                        continue;
                    }
                    if (isset($handled[$correlation])) {
                        continue;
                    }
    ...
    private function reconcileOne(array $event, string $jobName, string $awsStatus): void
    {
        ...
        if ((string)($result['status'] ?? '') !== 'COMPLETED') {
            throw new RuntimeException('AWS listó COMPLETED pero GetTranscriptionJob devolvió otro estado.');
        }
        ...
        $this->recordGeneratedS3Cost($userId, $fileId, $result, $started, $correlation);
    }
