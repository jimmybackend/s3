...
final class MoveJobController extends AbstractJsonController
{
    public function start(): never
    {
        try {
            ...
            if ($type === 'files') {
                ...
                if ($destination === '') {
                    $destination = $this->request->postString('ruta_destino');
                }
                ...
                $origin = $this->requireNonEmpty(
                    $this->request->postString('origen'),
                    'Falta la carpeta origen.'
                );
                $destination = $this->requireNonEmpty(
                    $this->request->postString('destino'),
    ...
    public function status(): never
    {
        try {
            ...
            if ($jobId === '') {
                $jobId = trim($this->request->postString('job_id'));
            }
            if ($jobId === '') {
                throw new RuntimeException('Falta el identificador de la tarea.');
            }
            ...
            if ($status === 'completed' && $type === 'folder') {
                ...
                if ($origin !== '' && $final !== '') {
                    ...
                    $current = $paths->normalizeForUser(
                        (string)$session->get('ruta_actual', $root),
                        $userId
                    );
            ...
            if ($destination !== '') {
                try {
                    $destinationLabel = $this->app->folderQueryService()
                        ->displayPathForUser($userId, $destination);
                } catch (\Throwable) {
                    $destinationLabel = '';
                }
            }
