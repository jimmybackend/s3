...
$chunkedUploadJs = $read('drive/js/subir-chunked.js');
$adminUploadPage = $read('drive/up.php');
...
$ok(str_contains($chunkedUploadJs, "'X-Drive-CSRF': CSRF"), 'chunked completion sends Drive CSRF header');
$completePos = strpos($chunkedUploadJs, "mode=chunked&action=complete");
$csrfAfterComplete = $completePos === false ? false : strpos($chunkedUploadJs, "'X-Drive-CSRF': CSRF", $completePos);
$ok($completePos !== false && $csrfAfterComplete !== false, 'chunked complete specifically carries Drive CSRF');
$ok(str_contains($chunkedUploadJs, 'moderation_blocked') && str_contains($chunkedUploadJs, 'clearSession(file)'), 'chunked UI clears stale resume state after moderation rejection');
$ok(str_contains($adminMultipart, "409"), 'admin multipart classifies blocked content as conflict');
