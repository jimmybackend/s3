...
// Instancia EC2 que sirve:
// http://IP_PUBLICA/palabra/palabras-v2
// http://IP_PUBLICA/palabra/palabras-3d
const PALABRA_INSTANCE_ID = 'i-0bc37c58f4adedad7';

// Archivo que lee /var/www/html/biblia/main05.php
const PALABRA_STATE_FILE = '/var/lib/deepbible/palabra-server.json';

// Ruta del proyecto dentro de la instancia t2.nano.
const PALABRA_BASE_PATH = '/palabra';
...
/**
 * Escribe el estado que consume main05.php.
 *
 * Cuando $active es true, debe existir una IPv4 pública válida.
 * La escritura se realiza mediante archivo temporal para evitar
 * que main05.php lea un JSON incompleto.
 */
function actualizar_estado_servidor_palabra(
    bool $active,
    ...
    $datos = [
        'active'      => $active,
        'instance_id' => PALABRA_INSTANCE_ID,
        'public_ip'   => $active ? $publicIp : null,
        'base_url'    => $active
            ? 'http://' . $publicIp . PALABRA_BASE_PATH
            : null,
        'updated_at'  => gmdate('c'),
    ];
    ...
    if ($json === false) {
        throw new RuntimeException(
            'No fue posible generar el JSON del servidor Palabra.'
        );
    }

    $directorio = dirname(PALABRA_STATE_FILE);

    if (!is_dir($directorio)) {
        throw new RuntimeException(
            'No existe el directorio de estado de Palabra: ' . $directorio
        );
    }

    if (!is_writable($directorio)) {
        throw new RuntimeException(
            'PHP no puede escribir en el directorio: ' . $directorio
        );
    }

    $archivoTemporal = PALABRA_STATE_FILE
        . '.'
        . getmypid()
        . '.'
    ...
    try {
        ...
        if ($bytes === false) {
            throw new RuntimeException(
                'No fue posible escribir el archivo temporal de Palabra.'
            );
        }

        @chmod($archivoTemporal, 0640);

        if (!rename($archivoTemporal, PALABRA_STATE_FILE)) {
            throw new RuntimeException(
                'No fue posible reemplazar el archivo de estado de Palabra.'
            );
        }
    } finally {
...
/**
 * Sincroniza el JSON sólo cuando la instancia consultada es Palabra.
 *
 * No interrumpe el funcionamiento general del panel si el archivo
 * no pudiera escribirse: registra el problema en el error_log.
 */
function sincronizar_estado_palabra(
    string $instanceId,
    string $state,
    string $publicIp
): void {
    if ($instanceId !== PALABRA_INSTANCE_ID) {
        return;
    }
    ...
    try {
        ...
        actualizar_estado_servidor_palabra(
            $active,
            $active ? $publicIp : null
        );
    } catch (Throwable $t) {
        error_log(
            'No se pudo sincronizar Palabra: ' . $t->getMessage()
        );
    }
}
...
/*
|--------------------------------------------------------------------------
| Sincronizar proyecto Palabra
|--------------------------------------------------------------------------
|
| El navegador consulta este endpoint mientras la instancia enciende
| o se detiene. Cuando AWS reporte running y entregue la IP pública,
| el archivo palabra-server.json quedará activo automáticamente.
|
*/

sincronizar_estado_palabra(
    $id,
    $st,
    $pip
);
...
try {
    ...
    if ($action === 'start') {
        /*
         * AWS aceptó la solicitud de encendido.
         * Mientras está pending, desactivamos el enlace viejo.
         * El polling lo activará cuando exista la nueva IP pública.
         */
        $panel->start($id);
        ...
        /*
         * AWS aceptó la solicitud de detención.
         * Desactivamos los enlaces inmediatamente para evitar
         * que alguien intente entrar mientras se apaga.
         */
        $panel->stop($id, $force);
...
try {
    ...
    /*
     * Sincronizar Palabra con la información que ya devolvió AWS
     * al cargar la tabla del panel.
     */
