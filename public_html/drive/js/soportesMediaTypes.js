...
(function(){
  ...
  /** Subida directa a S3 por URL presignada (PUT) */
  function subirBlob(blob, filename, mime){
    estado('Preparando URL firmada…');
    if (bar) {
      bar.parentElement && bar.parentElement.classList.remove('d-none');
      bar.style.width = '0%';
    }

    // 1) Solicitar URL firmada a PHP (usa la ruta de sesión en el servidor)
    fetch('firma_grabadora.php', {
      method: 'POST',
    ...
    .then(r => r.json())
    .then(json => {
      if (!json.ok || !json.url) throw new Error(json.error || 'No se pudo firmar');

      estado('Subiendo a S3…');
      // 2) PUT directo a S3 con XHR (para progreso)
      return new Promise((resolve, reject) => {
        const xhr = new XMLHttpRequest();
        xhr.open('PUT', json.url, true);
        xhr.setRequestHeader('Content-Type', mime); // Debe coincidir con lo firmado
    ...
    .then(json => {
      estado('Guardado en S3');
      if (window.actualizarBloqueArchivos) window.actualizarBloqueArchivos({ pagina: 1 });
      setTimeout(() => {
        try {
          if (typeof jQuery !== 'undefined' && jQuery(modalEl).modal) {
            jQuery(modalEl).modal('hide');       // BS4
          } else if (window.bootstrap?.Modal) {
            (bootstrap.Modal.getInstance(modalEl) || new bootstrap.Modal(modalEl)).hide(); // BS5
          }
        } catch(e){}
      }, 600);
    })
    .catch(err => {
      console.error(err);
      estado(err.message || 'Error subiendo a S3');
    });
  }
})();
