...
(function () {
  ...
  window.addEventListener('DOMContentLoaded', () => {
    ...
    function parseMarkdownTable(block) {
      ...
      if (!/^(\|?[:\-]+)+\|?[:\-]*$/.test(sep.replace(/\|/g, '|'))) return null;
      ...
      if (body.length) {
        html += '<tbody>';
        html += body.map(row => {
          const cells = head.map((_, i) => `<td>${row[i] || ''}</td>`).join('');
          return `<tr>${cells}</tr>`;
        }).join('');
        html += '</tbody>';
      }
      ...
      while (i < lines.length) {
        ...
        if (raw.trimStart().startsWith('```')) {
          if (!inCode) {
            ...
            out.push(
              `<pre class="chat-code-block"${langAttr}><code>${codeLines.join('\n')}</code></pre>`
            );
            inCode = false;
            codeLines = [];
            codeLang = '';
        ...
        while (i < lines.length) {
          ...
          if (!next) break;
          if (/^(#{1,6})\s+/.test(nextRaw)) break;
          if (/^\s*>\s?/.test(nextRaw)) break;
          if (/^\s*[-*]\s+/.test(nextRaw)) break;
          if (/^\s*\d+\.\s+/.test(nextRaw)) break;
          if (/^([-*_]){3,}$/.test(next.replace(/\s/g, ''))) break;
          if (nextRaw.trimStart().startsWith('```')) break;
        ...
        out.push(`<p>${mdInline(paragraph.join('<br>'))}</p>`);
      }

      if (inCode) {
        const langAttr = codeLang ? ` data-lang="${codeLang}"` : '';
        out.push(`<pre class="chat-code-block"${langAttr}><code>${codeLines.join('\n')}</code></pre>`);
      }
