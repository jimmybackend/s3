...
// Cambia el estado de una sesión a 'archived'.
// POST: session_id (int)
...
function is_admin_like($role) {
    ...
    // Ajusta la lista a tu gusto (según tus roles)
...
if (!$can_edit) {
    jexit(['ok' => false, 'error' => 'No tienes permisos para archivar esta sesión'], 403);
}

/* ============================
   Actualizar estado => 'archived'
   ============================ */
if ($owner_id === $user_id || !is_admin_like($role)) {
    // caso normal: asegurar dueño en el WHERE
    $sqlUp = "UPDATE ChatSessions SET status = 'archived' WHERE id_ = ? AND user_id_ = ? LIMIT 1";
    ...
    $sqlUp = "UPDATE ChatSessions SET status = 'archived' WHERE id_ = ? LIMIT 1";
    $stmtU = $db_connection->prepare($sqlUp);
    if (!$stmtU) jexit(['ok' => false, 'error' => 'Error preparando UPDATE: ' . $db_connection->error], 500);
