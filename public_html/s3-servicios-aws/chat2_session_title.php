...
// POST: session_id (int), title (string)
// RESP: { ok, id, title, status, model_id, provider, updated_at }
...
function is_admin_like($role) {
    ...
    // Puedes ajustar la lista de roles con privilegios
...
if (!$can_edit) {
    jexit(['ok' => false, 'error' => 'No tienes permisos para renombrar esta sesión'], 403);
}
...
if ($owner_id === $user_id || !is_admin_like($role)) {
    ...
    $sqlUp = "UPDATE ChatSessions SET title = ? WHERE id_ = ? AND user_id_ = ? LIMIT 1";
    $stmtU = $db_connection->prepare($sqlUp);
    if (!$stmtU) jexit(['ok' => false, 'error' => 'Error preparando UPDATE: ' . $db_connection->error], 500);
    $stmtU->bind_param('sii', $title, $session_id, $owner_id);
} else {
    // admin/soporte puede actualizar sin condicionar por user_id_
    $sqlUp = "UPDATE ChatSessions SET title = ? WHERE id_ = ? LIMIT 1";
    $stmtU = $db_connection->prepare($sqlUp);
    if (!$stmtU) jexit(['ok' => false, 'error' => 'Error preparando UPDATE: ' . $db_connection->error], 500);
    $stmtU->bind_param('si', $title, $session_id);
}
...
// Releer fila actualizada para devolver updated_at
...
// Fallback si no pudimos releer (debería ser raro)
jexit([
    'ok'    => true,
    'id'    => (int)$session_id,
    'title' => $title
