...
<body class="ui-theme theme-neon-green theme-dark vision-normal ascii-on">
      ...
      <div class="row align-items-center g-4">
        <div class="col-lg-7">
          ...
          <div class="d-flex flex-wrap gap-2">
            <span class="badge badge-dark-custom">S3</span>
            <span class="badge badge-dark-custom">Rekognition</span>
            <span class="badge badge-dark-custom">Polly</span>
            <span class="badge badge-dark-custom">Translate</span>
            <span class="badge badge-dark-custom">KMS/Encrypt</span>
            <span class="badge badge-dark-custom">ZIP</span>
          </div>
        </div>
          ...
          <div class="card shadow-sm h-100">
            <div class="card-body">
              ...
              <div class="row row-cols-2 g-2 mt-1">
                <div class="col">
                  <button class="btn btn-outline-primary w-100" onclick="abrirModal('modalSubir')">
                    <i class="bi bi-cloud-arrow-up me-1"></i>Subir
                  </button>
                </div>
                <div class="col">
                  <button class="btn btn-outline-secondary w-100" onclick="refrescarLista()">
                    <i class="bi bi-arrow-clockwise me-1"></i>Refrescar
                  </button>
                </div>
                <div class="col">
                  <button class="btn btn-outline-secondary w-100" onclick="compactarZIPSeleccion()">
                    <i class="bi bi-file-zip me-1"></i>Compactar ZIP
                  </button>
                </div>
                <div class="col">
                  <button class="btn btn-outline-secondary w-100" onclick="descompactarZIPSeleccion()">
                    <i class="bi bi-folder-symlink me-1"></i>Descompactar
                  </button>
                </div>
              </div>
            </div>
  ...
  <div class="position-relative">
    ...
    <div class="d-flex align-items-center mb-2">
      ...
      <h3 class="h5 mb-0">
        <a href="videollamada.php" class="text-decoration-none text-white">Video Llamada</a>
      </h3>
    </div>

    <ul class="mb-2">
      <!-- Enlace directo a la página de videollamada -->
      <li><a href="videollamada.php" class="text-white text-decoration-underline">Llamar desde la Web</a></li>
    </ul>
  ...
  <a href="videollamada.php" class="stretched-link" aria-label="Ir a videollamada"></a>
</div>
  ...
  <section id="archivos" class="py-4">
    <div class="container">
      <div class="archivos-wrap p-3 p-md-4 rounded-3">

        <div class="d-flex flex-column flex-lg-row align-items-lg-center justify-content-between gap-3 mb-3">
          <div>
            <h2 class="h4 mb-1">Archivos</h2>
            <div class="small text-soft">Explora, filtra y administra tus archivos</div>
          </div>
        ...
        <div class="table-responsive">
          <table class="table table-striped table-hover align-middle mb-0" id="tablaArchivos">
            <thead>
              <tr>
                <th style="width:40px">
                ...
                <th>Nombre</th>
                <th class="text-center" style="width:100px;">Ext</th>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div class="small text-soft">
            Genera una URL prefirmada desde tu endpoint PHP y cópiala para compartir.
          </div>
        </div>
        <div class="modal-footer">
          <button class="btn btn-outline-secondary" data-bs-dismiss="modal">Cerrar</button>
          <button class="btn btn-primary" id="btnCopyShare">
...
<div class="toast-container position-fixed top-0 end-0 p-3">
  ...
  <script>
    ...
    function renderTabla() {
      ...
      const rows = archivos.filter(a =>
        (!q || a.nombre.toLowerCase().includes(q)) &&
        (!fext || a.ext.toLowerCase() === fext)
      );
      ...
      tbody.innerHTML = rows.map((a, idx) => `
        <tr data-key="${a.key}" data-ext="${a.ext}" data-nombre="${a.nombre}" data-ruta="${a.ruta}">
          ...
          <td class="pointer" onclick="preview(${idx})">
            <i class="bi bi-file-earmark me-1"></i>${a.nombre}
          </td>
          <td class="text-center">
            <span class="badge badge-ext-custom">${a.ext}</span>
          </td>
          <td class="text-end">${bytes(a.size)}</td>
          <td class="text-center">
            <div class="btn-group btn-group-sm">
              <button class="btn btn-outline-secondary" title="Ver" onclick="verArchivo('${a.ext}','${encodeURIComponent(a.key)}')">
                <i class="bi bi-eye"></i>
              </button>
              <button class="btn btn-outline-secondary" title="Editar texto" onclick="editarTxt('${encodeURIComponent(a.key)}')">
                <i class="bi bi-pencil"></i>
              </button>
              <button class="btn btn-outline-secondary" title="Descargar" onclick="descargar('${encodeURIComponent(a.key)}')">
                <i class="bi bi-download"></i>
              </button>
              <button class="btn btn-outline-secondary" title="Renombrar / Copiar" onclick="abrirRenombrar('${a.nombre}')">
                <i class="bi bi-file-earmark-diff"></i>
              </button>
              <button class="btn btn-outline-secondary" title="Mover" onclick="abrirModalMover('${encodeURIComponent(a.key)}')">
                <i class="bi bi-arrows-move"></i>
              </button>
              <button class="btn btn-outline-secondary" title="Compartir" onclick="compartirArchivo('${encodeURIComponent(a.key)}','${a.ext}')">
                <i class="bi bi-share"></i>
              </button>
              <button class="btn btn-outline-secondary" title="Seguridad" onclick="toggleLock('${encodeURIComponent(a.key)}')">
                <i class="bi bi-shield-lock"></i>
              </button>
              <button class="btn btn-outline-secondary" title="Eliminar" onclick="eliminarArchivo('${encodeURIComponent(a.key)}','${encodeURIComponent(a.ruta)}')">
                <i class="bi bi-trash"></i>
              </button>
              <button class="btn btn-outline-secondary" title="ZIP" onclick="zipAccion('${encodeURIComponent(a.key)}')">
                <i class="bi bi-file-zip"></i>
              </button>
              <button class="btn btn-outline-secondary" title="IA" onclick="abrirIA('${encodeURIComponent(a.key)}','${a.nombre}','${a.ext}')">
                <i class="bi bi-cpu"></i>
              </button>
            </div>
          </td>
    ...
    document.getElementById('chkAll').addEventListener('change', (e) => {
      document.querySelectorAll('.chkRow').forEach(ch => {
    ...
    function preview(idx) {
      ...
      verArchivo(a.ext, encodeURIComponent(a.key));
    }
    ...
    function editarTxt(s3key) {
      showToast('Editar texto: ' + s3key);
    }

    function verPDF(s3key) {
      showToast('Ver PDF: ' + s3key);
    }

    function verAudioDesde(s3key) {
      showToast('Reproducir audio: ' + s3key);
    }

    function descargar(key) {
      key = decodeURIComponent(key);
      const url = 'descargar.php?archivo=' + encodeURIComponent(key);
      window.open(url, '_blank');
    }

    function eliminarArchivo(key, ruta) {
      key = decodeURIComponent(key);
      ruta = decodeURIComponent(ruta);

      if (confirm('¿Eliminar "' + key + '"?')) {
        showToast('Solicitud de eliminación enviada: ' + key);
      }
    }

    function abrirRenombrar(nombre) {
      document.getElementById('oldName').value = nombre;
      document.getElementById('newName').value = nombre;
      abrirModal('modalRenombrarArchivo');
    }
    ...
    document.getElementById('btnConfirmRename').addEventListener('click', () => {
      ...
      if (!newName) {
        return showToast('Escribe un nombre válido.');
      }
    ...
    function abrirModalMover(s3key) {
      moverKey = decodeURIComponent(s3key);
    ...
    document.getElementById('btnConfirmMover').addEventListener('click', () => {
      ...
      if (!dest) {
        return showToast('Define una ruta destino.');
      }
    ...
    function compartirArchivo(key, ext) {
      key = decodeURIComponent(key);
      const demo = location.origin + '/share/presigned/' + encodeURIComponent(key);
      document.getElementById('inputShare').value = demo;
      abrirModal('modalCompartir');
    }

    document.getElementById('btnCopyShare').addEventListener('click', async () => {
      const valor = document.getElementById('inputShare').value;
    ...
    // ==================== Seguridad ====================
    function setFileSecurity(action, key) {
      showToast('Seguridad: ' + action + ' => ' + key);
    }

    function toggleLock(key) {
      key = decodeURIComponent(key);
      setFileSecurity('toggle', key);
    }

    // ==================== ZIP ====================
    function zipAccion(key) {
      key = decodeURIComponent(key);
      showToast('Acción ZIP sobre: ' + key);
    }

    function compactarZIPSeleccion() {
      showToast('Compactar selección a ZIP');
    }

    function descompactarZIPSeleccion() {
      showToast('Descompactar ZIP de la selección');
    }

    // ==================== IA / AWS ====================
    function extraerTexto(s3key) {
      showToast('DOC2TXT para: ' + s3key);
    }

    function transcribirAudio(s3key) {
      showToast('AUDIO2TXT para: ' + s3key);
    }

    function abrirModalPolly(s3key) {
      abrirModal('modalPolly');
      document.getElementById('pollyText').value = 'Texto ejemplo para ' + s3key;
    }

    function abrirModalTraducir(s3key, nombre) {
      abrirModal('modalTraducir');
      document.getElementById('traducirText').value = 'Contenido de ' + (nombre || s3key);
    }

    function abrirModalRekognition(s3key) {
      abrirModal('modalRekognition');
      document.getElementById('rekognitionParams').value = '{"labels":true,"moderation":true}';
    }
    ...
    function abrirIA(key, nombre, ext) {
      ...
      if (['jpg','jpeg','png','gif','webp'].includes(ext)) {
        return abrirModalRekognition(key);
      }

      if (['mp3','wav','m4a','aac'].includes(ext)) {
        return transcribirAudio(key);
      }

      if (['txt','md','csv'].includes(ext)) {
        return abrirModalPolly(key);
      }
    ...
    document.getElementById('btnRunPolly').addEventListener('click', () => {
      ...
      if (!text) {
        return showToast('Escribe texto para convertir.');
      }
    ...
    document.getElementById('btnRunTraducir').addEventListener('click', () => {
      ...
      if (!text) {
        return showToast('Escribe texto a traducir.');
      }
    ...
    document.getElementById('btnEnviarSubida').addEventListener('click', () => {
      ...
      if (!files.length) {
        return showToast('Selecciona archivos.');
      }
    ...
    dropzone.addEventListener('drop', e => {
      ...
      showToast(files.length + ' archivo(s) listos para subir.');
    });
