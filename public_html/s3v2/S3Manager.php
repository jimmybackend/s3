...
class S3Manager
{
    ...
    private function renameMoveFolderTreeDb(int $userId, string $oldPrefix, string $newPrefix): array
    {
        ...
        foreach ($folders as $folder) {
            $currentPrefix = $this->normalizePrefix($folder['Prefix']);
            $newFolderPrefix = $newPrefix . substr($currentPrefix, strlen($oldPrefix));
    
            // Carpeta raíz movida/renombrada
            if ($currentPrefix === $oldPrefix) {
                $newParent = $this->parentPrefix($newPrefix);
            } else {
                // Subcarpetas hijas
                $currentParent = $folder['ParentPrefix'] !== null
                    ? $this->normalizePrefix($folder['ParentPrefix'])
                    : null;
    
                $newParent = null;
    
                if ($currentParent !== null) {
                    if (strpos($currentParent, $oldPrefix) === 0) {
                        $newParent = $newPrefix . substr($currentParent, strlen($oldPrefix));
                    } else {
        ...
        /**
         * IMPORTANTE:
         * En FileS3 solo debe cambiar la Ruta.
         *
         * NO se debe modificar:
         * - Nombre      -> nombre visible en BD
         * - Encriptado  -> nombre real del archivo en S3
         *
         * Así la key real sigue siendo:
         * nueva Ruta + mismo Encriptado
         */
        $startRuta = strlen($oldPrefix) + 1;
    
        $sqlUpdFiles = "UPDATE FileS3
                        SET Ruta  = CONCAT(?, SUBSTRING(Ruta, ?)),
                            Found = 1
                        WHERE user_id_ = ?
                          AND (Ruta = ? OR Ruta LIKE CONCAT(?, '%'))";
    
        $stmtUpdFiles = $this->db->prepare($sqlUpdFiles);
        if (!$stmtUpdFiles) {
            throw new RuntimeException('Error preparando update de FileS3: ' . $this->db->error);
        }
    
        $stmtUpdFiles->bind_param(
            'sisss',
            $newPrefix,
            $startRuta,
            $userId,
        ...
        $this->executeStmt($stmtUpdFiles, 'Error actualizando FileS3');
        $filesUpdated = $stmtUpdFiles->affected_rows;
        $stmtUpdFiles->close();
    ...
    public function eliminarCarpetaCompleta(string $prefijo): void
    {
        $userId = $this->resolveUserId();
        $prefijo = $this->normalizePrefix($prefijo);

        $base = $this->getBasePrefix();
        if ($prefijo === $base) {
            throw new RuntimeException('No puedes eliminar la carpeta raíz.');
        }
        ...
        do {
            ...
            $archivos = $this->s3->listObjectsV2($params);

            foreach ($archivos['Contents'] ?? [] as $objeto) {
                $toDelete[] = ['Key' => $objeto['Key']];
            ...
            $continuationToken = !empty($archivos['IsTruncated'])
                ? ($archivos['NextContinuationToken'] ?? null)
                : null;
        ...
        try {
            $this->deleteFolderTreeDb($userId, $prefijo);
            $this->db->commit();
        } catch (Throwable $e) {
            $this->db->rollback();
            throw $e;
        ...
        $this->updateSessionRouteAfterDelete($prefijo);
    }
...
public function renombrarCarpeta(string $rutaAntigua, string $nuevoNombre): void
{
    ...
    public function renameFile($fileRef, $nuevoNombre, $rutaActual = null)
    {
        ...
        $id = (int)$file['id_'];
        $stmt->bind_param('sssi', $nuevoNombre, $nuevoEncriptado, $ruta, $id);
     ...
     * inmediatamente la columna Ruta en FileS3. Acepta id_ o KEY S3.
     *
     * PARÁMETROS:
     * $fileRef   → id_ o key S3
    ...
    public function moveFile($fileRef, $nuevaRuta)
    {
        ...
        $id = (int)$file['id_'];
        $stmt->bind_param('si', $nuevaRuta, $id);
     ...
     * ============================================================
     * DESCRIPCIÓN:
     * Mueve múltiples archivos a una nueva carpeta usando las KEYs S3
     * recibidas desde la interfaz.
     *
     * PARÁMETROS:
     * $keys      → arreglo de keys S3
     * $nuevaRuta → carpeta destino
     *
     * RETORNA:
     * array con cantidad procesada y ruta destino
     * ============================================================
     */
