...
(function (window, document) {
  ...
  async function refrescarBloqueCarpetasCompat() {
    // Si ya tienes un actualizador global, úsalo
    if (typeof window.actualizarBloqueCarpetas === 'function') {
      try {
        const p = window.actualizarBloqueCarpetas();
        if (p && p.then) await p;
        return;
      } catch (e) {
        console.warn('[carpetas.js] actualizarBloqueCarpetas falló. Fallback fetch.', e);
      }
    }

    const cont = $id(CFG.ids.bloqueCarpetas);
    if (!cont) return;

    const html = await fetchNoCache(CFG.urls.bloqueCarpetas, { method: 'GET' }).then(r => r.text());
    cont.innerHTML = html;


    // Ejecuta scripts inline (si el bloque trae alguno)
    runInlineScripts(cont);
  ...
  async function abrirRuta(ruta, force = false) {
    ...
    // evita doble carga (muy común con 2 handlers o doble click)
    if (!force && ruta === ultimaRutaAbierta) return;
    ultimaRutaAbierta = ruta;

    // Limpieza suave (tu bindArbolCarpetas.js lo hacía)
    try { if (window.imagenesGaleria) window.imagenesGaleria = []; } catch (_) {}

    await actualizarRutaSesion(ruta);
    marcarSeleccionEnArbol(ruta);

    // cargarCarpetas (compatibilidad) si existe y NO es esta misma (evita loop)
    // OJO: nosotros exponemos window.cargarCarpetas, así que aquí llamaríamos a nosotros mismos.
    // Por eso mejor refrescamos directamente el bloque archivos.
    const filtros = leerFiltrosActualesDelDOM();
    filtros.ruta = ruta;
    await refrescarBloqueArchivosCompat(filtros);

    // footer opcional (tu cargarcarpetas.js lo hacía)
    if (typeof window.actualizarBloqueFooter === 'function') {
      try { window.actualizarBloqueFooter(); } catch (_) {}
    }
  }
  ...
  async function onClickMover() {
    ...
    if (!origen) { alert('Falta la carpeta de origen.'); return; }
    if (!destino) { alert('Selecciona una carpeta de destino.'); return; }

    // Previene mover dentro de sí misma/subcarpeta (igual que tu mover-carpeta.js)
    ...
    try {
      ...
      const j = await res.json().catch(() => ({}));
      if (!j.ok) throw new Error(j.error || 'No se pudo mover la carpeta.');

      safeHideModal(modal);

      await refrescarBloqueCarpetasCompat();
      await refrescarBloqueArchivosCompat({ pagina: 1 });
  ...
  async function onSubmitRenombrar(e) {
    ...
    try {
      ...
      if (!res.ok || !j) {
        ...
        // asegúrate de que los nombres coinciden con tu s3.php: renombrar=1, nombre_actual, nuevo_nombre
        // tu modal ya los trae.
        res = await fetchNoCache(action, { method: 'POST', body: fd });
        if (!res.ok) throw new Error('No se pudo renombrar la carpeta (HTTP ' + res.status + ').');
      } else if (j.ok !== true) {
...
function initNuevaRutaSelect(scope) {
  ...
  async function onSubmitEliminar(e) {
    ...
    try {
      ...
      const j = await res.json().catch(() => ({}));
      if (!j.ok) throw new Error(j.error || 'No se pudo eliminar la carpeta.');
...
async function onSubmitCrearCarpeta(e) {
  ...
  if (!nueva) { creandoCarpeta = false; return; }
  ...
  try {
    ...
    // 1) Preferido: endpoint dedicado crear_carpeta.php (devuelve JSON)
    if (CFG.urls && CFG.urls.crearCarpeta) {
      ...
      if (!res.ok || !j) throw new Error('Respuesta inválida creando carpeta.');
      if (!j.ok) throw new Error(j.error || 'No se pudo crear la carpeta.');
    ...
    // 2) Fallback: postear al action del form (s3.php u otro)
    const action = form.getAttribute('action') || window.location.href;
    const res2 = await fetchNoCache(action, { method: 'POST', body: fd, credentials: 'same-origin' });
    if (!res2.ok) throw new Error('No se pudo crear la carpeta (HTTP ' + res2.status + ').');

    // Limpia input sin resetear todo si quieres:
    const input = form.querySelector('input[name="nueva"]');
...
function bindModalEvents() {
  // Bootstrap 5: eventos nativos en el DOM
  ...
  // ✅ Cuando el modal ya se mostró, ajustamos el estado del select/campo (si existe)
  document.addEventListener('shown.bs.modal', function () {
    // No asume que exista; la función ya debe ser segura con null-checks
    initNuevaRutaSelect();
  }, true);

  // Bootstrap 4: si existe jQuery, también nos enganchamos (para asegurar evt.relatedTarget)
  if (window.jQuery) {
    try {
      window.jQuery(document).off('show.bs.modal.carpetasjs');
      window.jQuery('#' + CFG.ids.modalMoverCarpeta).off('show.bs.modal.carpetasjs').on('show.bs.modal.carpetasjs', onShowMoverModal);
      window.jQuery('#' + CFG.ids.modalRenombrar).off('show.bs.modal.carpetasjs').on('show.bs.modal.carpetasjs', onShowRenombrarModal);
      window.jQuery('#' + CFG.ids.modalEliminarCarpeta).off('show.bs.modal.carpetasjs').on('show.bs.modal.carpetasjs', onShowEliminarModal);

      // ✅ Bootstrap 4 shown
      window.jQuery(document).off('shown.bs.modal.carpetasjs').on('shown.bs.modal.carpetasjs', function () {
        initNuevaRutaSelect();
      });
    } catch (_) {}
  }
