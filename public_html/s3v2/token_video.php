<?php
// token_video.php — público (GET ?t=...) y legacy API (POST archivo)
...
if ($t !== '') {
  ...
  // (opcional) puedes validar tipo: $tok['tipo'] === 'video'
...
// ¿JSON en GET? (opcional para depuración)
if (isset($_GET['json'])) {
  header('Content-Type: application/json; charset=UTF-8');
...
<title><?= htmlspecialchars($nombre) ?> · Video compartido</title>
<style>
...
<body>
  <div class="wrap">
    ...
    <div class="box">
      <video controls playsinline preload="metadata" src="<?= htmlspecialchars($url) ?>"></video>
    </div>
    <div class="meta">
